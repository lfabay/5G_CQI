...
def 渲染制式对比距离覆盖分析(分析器: CQI分析器):
    ...
    with col_hist1:
        if 'n41' in 覆盖系数统计:
            ...
            fig_n41.add_vline(x=0.3, line_dash="dash", line_color="green", annotation_text="适中下限")
            fig_n41.add_vline(x=0.7, line_dash="dash", line_color="orange", annotation_text="适中上限")
            fig_n41.add_vline(x=1.0, line_dash="dash", line_color="red", annotation_text="越区阈值")
            fig_n41.update_layout(template="plotly_white", height=400)
            st.plotly_chart(fig_n41, use_container_width=True)
    ...
    with col_hist2:
        if 'n28' in 覆盖系数统计:
            ...
            fig_n28.add_vline(x=0.3, line_dash="dash", line_color="green", annotation_text="适中下限")
            fig_n28.add_vline(x=0.7, line_dash="dash", line_color="orange", annotation_text="适中上限")
            fig_n28.add_vline(x=1.0, line_dash="dash", line_color="red", annotation_text="越区阈值")
            fig_n28.update_layout(template="plotly_white", height=400)
            st.plotly_chart(fig_n28, use_container_width=True)
...
def main():
    """主函数"""
    st.markdown('<p class="main-header">📡 5G CQI关联性能分析系统<br><small>网络制式对比版 (N41 vs N28) - 中兴数据版</small></p>', unsafe_allow_html=True)
