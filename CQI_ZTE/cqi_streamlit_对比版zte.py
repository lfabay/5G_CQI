...
def 渲染制式对比速率影响(分析器: CQI分析器):
    ...
    if 'n41' in 速率影响 and 'n28' in 速率影响:
        总结内容 = "<div class='highlight'><b>📊 CQI对速率影响分析总结</b><br><br>"
        ...
        for 速率 in 速率影响['n41'].keys():
            if 速率 in 速率影响['n28']:
                ...
                总结内容 += f"• {速率}: N41相关系数={n41_r:.4f}, N28相关系数={n28_r:.4f}<br>"

        总结内容 += "<br><b>2. 显著性分析：</b><br>"
        for 速率 in 速率影响['n41'].keys():
            if 速率 in 速率影响['n28']:
                n41_sig = 速率影响['n41'][速率]['显著性']
                n28_sig = 速率影响['n28'][速率]['显著性']
                总结内容 += f"• {速率}: N41={n41_sig}, N28={n28_sig}<br>"

        总结内容 += "<br><b>3. 关键发现：</b><br>"
        # 找出相关性最强的指标
        n41最强 = max(速率影响['n41'].items(), key=lambda x: abs(x[1]['相关系数']))
        n28最强 = max(速率影响['n28'].items(), key=lambda x: abs(x[1]['相关系数']))
        总结内容 += f"• N41与CQI相关性最强的指标: {n41最强[0]} (相关系数={n41最强[1]['相关系数']:.4f})<br>"
        总结内容 += f"• N28与CQI相关性最强的指标: {n28最强[0]} (相关系数={n28最强[1]['相关系数']:.4f})<br>"

        总结内容 += "<br><b>4. 优化建议：</b><br>"
        总结内容 += "• 关注CQI对下行/上行速率影响较大的网络制式<br>"
        总结内容 += "• 根据相关性强度制定针对性的CQI优化策略<br>"
        总结内容 += "• 优先优化相关性高的指标可获得更好的整体性能提升"
...
def 渲染制式对比阈值分析(分析器: CQI分析器):
    ...
    n28有数据 = 'n28' in 阈值分析结果 and len(阈值分析结果['n28']) > 0
    
    if n41有数据 or n28有数据:
        总结内容 = "<div class='success-box'><b>💡 阈值分析对比结论：</b><br>"
        
        if n41有数据:
            最大增益_n41 = 阈值分析结果['n41'].loc[阈值分析结果['n41']['速率增益'].idxmax()]
            总结内容 += f"<b>N41最优阈值</b>：CQI阈值为<strong>{最大增益_n41['CQI阈值']:.2f}%</strong>时，速率增益最大，达到<strong>{最大增益_n41['速率增益']:.2f} Mbps</strong>（{最大增益_n41['增益百分比']:.2f}%）<br>"
            总结内容 += f"&nbsp;&nbsp;• 高于阈值样本：{最大增益_n41['高于阈值样本数']:.0f} ({最大增益_n41['高于阈值占比(%)']:.1f}%) | 低于阈值样本：{最大增益_n41['低于阈值样本数']:.0f} ({最大增益_n41['低于阈值占比(%)']:.1f}%)<br><br>"
        
        if n28有数据:
            最大增益_n28 = 阈值分析结果['n28'].loc[阈值分析结果['n28']['速率增益'].idxmax()]
            总结内容 += f"<b>N28最优阈值</b>：CQI阈值为<strong>{最大增益_n28['CQI阈值']:.2f}%</strong>时，速率增益最大，达到<strong>{最大增益_n28['速率增益']:.2f} Mbps</strong>（{最大增益_n28['增益百分比']:.2f}%）<br>"
            总结内容 += f"&nbsp;&nbsp;• 高于阈值样本：{最大增益_n28['高于阈值样本数']:.0f} ({最大增益_n28['高于阈值占比(%)']:.1f}%) | 低于阈值样本：{最大增益_n28['低于阈值样本数']:.0f} ({最大增益_n28['低于阈值占比(%)']:.1f}%)<br><br>"
        
        if n41有数据 and n28有数据:
            对比结论 = "N41" if 最大增益_n41['速率增益'] > 最大增益_n28['速率增益'] else "N28"
            总结内容 += f"<b>📊 对比分析</b>：{对比结论}的CQI优化效果更显著，建议优先优化该制式的小区"
        elif n41有数据:
            总结内容 += "<b>📊 对比分析</b>：仅N41数据满足分析条件"
        elif n28有数据:
            总结内容 += "<b>📊 对比分析</b>：仅N28数据满足分析条件"
...
def 渲染制式对比分组分析(分析器: CQI分析器):
    """渲染分组分析的网络制式对比"""
    分组数 = st.slider("选择分组数", 3, 10, 5, key="group_slider")
