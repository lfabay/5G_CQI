    ...
    def 阈值分析_按制式(self, 阈值数: int = 10) -> Dict:
        ...
        for 制式, 数据 in 分组数据.items():
            ...
            for 阈值 in 阈值列表:
                ...
                if len(高于阈值数据) > 0 and len(低于阈值数据) > 0:
                    结果列表.append({
                        'CQI阈值': 阈值,
...
def 渲染制式对比阈值分析(分析器: CQI分析器):
    ...
    <div class="highlight">
    <b>🎯 阈值分析说明：</b><br>
    通过设置不同的CQI阈值，分析高于和低于该阈值的小区在速率上的差异，从而明确CQI优化的目标和预期增益。
    </div>
    """, unsafe_allow_html=True)
    ...
    # 对比总结
    if 'n41' in 阈值分析结果 and 'n28' in 阈值分析结果:
        ...
        <b>💡 阈值分析对比结论：</b><br>
        <b>N41最优阈值</b>：CQI阈值为<strong>{:.2f}%</strong>时，速率增益最大，达到<strong>{:.2f} Mbps</strong>（{:.2f}%）<br>
        <b>N28最优阈值</b>：CQI阈值为<strong>{:.2f}%</strong>时，速率增益最大，达到<strong>{:.2f} Mbps</strong>（{:.2f}%）<br>
        <b>对比分析</b>：{}的CQI优化效果更显著，建议优先优化该制式的小区
        </div>
        """.format(
