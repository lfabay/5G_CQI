...
def 生成综合报告(分析器: CQI分析器) -> str:
            ...
            if 分位数数据 and ('n41' in 分位数数据 or 'n28' in 分位数数据):
                ...
                if 'n41' in 分位数数据:
                    ...
                    if len(df_n41) > 0:
                        ...
                        报告.append(f"    N41: CQI从低到高，下行速率 {起始下行:.2f}→{结束下行:.2f} Mbps (提升{提升率:.1f}%)")
                if 'n28' in 分位数数据:
                    df_n28 = 分位数数据['n28']
                    if len(df_n28) > 0:
                        ...
                        提升率 = ((结束下行 - 起始下行) / 起始下行 * 100) if 起始下行 > 0 else 0
                        报告.append(f"    N28: CQI从低到高，下行速率 {起始下行:.2f}→{结束下行:.2f} Mbps (提升{提升率:.1f}%)")
                报告.append("")
    ...
    if 'n41' in 统计摘要 and 'n28' in 统计摘要:
        ...
        报告.append("  • CQI与速率强相关，提升CQI可有效改善用户体验")
...
def 渲染制式对比数据导出(分析器: CQI分析器):
    ...
    • N41_拐点分析 / N28_拐点分析
    • N41_贡献度分析 / N28_贡献度分析
    • N41_分组分析 / N28_分组分析（含覆盖系数、重叠覆盖采样点比例）
    • N41_覆盖系数统计 / N28_覆盖系数统计
    • N41_覆盖系数相关性 / N28_覆盖系数相关性
    • N41_覆盖系数分层分析 / N28_覆盖系数分层分析
    • N41三维散点图数据 / N28三维散点图数据（含覆盖系数、重叠覆盖采样点比例）
    • N41四象限分析 / N28四象限分析
    • N41_CQI不达标根因 / N28_CQI不达标根因
    """)
